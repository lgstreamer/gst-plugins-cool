...
GST_START_TEST (test_mpegts_pmt)
{
  ...
  pmt = (GstDvrMpegtsPMT *) gst_mpegts_section_get_pmt (pmt_section);
  ...
  desc = (GstDvrMpegtsDescriptor *) gst_mpegts_find_descriptor (pmt->descriptors,
  ...
  for (i = 0; i < 2; i++) {
    ...
    desc =
        (GstDvrMpegtsDescriptor *) gst_mpegts_find_descriptor (stream->descriptors,
        GST_MTS_DESC_REGISTRATION);
    fail_if (desc == NULL);
  }
...
GST_START_TEST (test_mpegts_nit)
{
  ...
  nit = (GstDvrMpegtsNIT *) gst_mpegts_section_get_nit (nit_section);
  ...
  desc = (GstDvrMpegtsDescriptor *) gst_mpegts_find_descriptor (nit->descriptors,
...
GST_START_TEST (test_mpegts_sdt)
{
  ...
  sdt = (GstDvrMpegtsSDT *) gst_mpegts_section_get_sdt (sdt_section);
...
GST_START_TEST (test_mpegts_descriptors)
{
  ...
  gst_dvrmpegts_descriptor_free (desc);
}
