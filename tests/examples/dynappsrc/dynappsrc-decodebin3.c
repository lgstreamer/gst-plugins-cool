/* GStreamer
 *
 * dynappsrc-decodebin3.c: example for using dynappsrc in decodebin3.
 *
...
GST_DEBUG_CATEGORY (dynappsrc_decodebin3_debug);
#define GST_CAT_DEFAULT dynappsrc_decodebin3_debug
...
read_data (App * app)
{
  ...
  GST_DEBUG ("feed buffer %p, offset %" G_GUINT64_FORMAT "-%u", buffer,
      app->offset, len);
  g_message ("feed buffer %p, offset %" G_GUINT64_FORMAT "-%u \n", buffer,
      app->offset, len);
...
setup_source (void)
{
  ...
  g_signal_emit_by_name (video_app->dynappsrc, "new-appsrc", "video",
      &video_app->appsrc);
  g_signal_emit_by_name (video_app->dynappsrc, "new-appsrc", "audio",
      &audio_app->appsrc);
...
static void
dynappsrc_pad_added_cb (GstElement * element, GstPad * pad, gpointer u_data)
{
  App *data = &v_app;
  GstPad *sinkpad = NULL;
  GstPadLinkReturn res;

  g_message ("pad added in dynappsrc");

  /* Primary stream, link to the main pad of decodebin3 */
  sinkpad = gst_element_get_static_pad (data->decodebin, "sink");
  ...
  if (sinkpad == NULL) {
    /* Auxiliary stream, request a new pad from decodebin3 */
    sinkpad = gst_element_get_request_pad (data->decodebin, "sink_%u");
  }

  if (sinkpad) {
    GST_DEBUG_OBJECT (data->pipeline, "New pad %" GST_PTR_FORMAT
        " from src %" GST_PTR_FORMAT " linking to %"
        GST_PTR_FORMAT, pad, element, sinkpad);

    res = gst_pad_link (pad, sinkpad);
    gst_object_unref (sinkpad);

    if (GST_PAD_LINK_FAILED (res))
      g_printerr ("failed to link pad %s:%s to decodebin, reason %s (%d)",
          GST_DEBUG_PAD_NAME (pad), gst_pad_link_get_name (res), res);
  }
}

static void
decodebin_pad_added_cb (GstElement * dbin, GstPad * pad, App * data)
{
...
main (int argc, char *argv[])
{
  ...
  GST_DEBUG_CATEGORY_INIT (dynappsrc_decodebin3_debug, "dynappsrc-decodebin3",
      0, "dynappsrc decodebin3 example");
  ...
  /* Create pipeline */
  video_app->pipeline = audio_app->pipeline = gst_pipeline_new ("pipeline");
  g_assert (video_app->pipeline);
  video_app->dynappsrc = audio_app->dynappsrc =
      gst_element_make_from_uri (GST_URI_SRC, "dynappsrc://", "source", NULL);
  g_assert (video_app->dynappsrc);
  video_app->decodebin = audio_app->decodebin =
      gst_element_factory_make ("decodebin3", NULL);
  g_assert (video_app->decodebin);
  video_app->playsink = audio_app->playsink =
      gst_element_factory_make ("playsink", NULL);
  g_assert (video_app->playsink);

  /* Enable to use stream-collection */
  s = gst_structure_new ("smart-properties", "use-stream-collection",
      G_TYPE_BOOLEAN, TRUE, NULL);
  g_object_set (video_app->dynappsrc, "smart-properties", s, NULL);

  /* Connect signals from source and decodebin elements */
  g_signal_connect (video_app->dynappsrc, "pad-added",
      G_CALLBACK (dynappsrc_pad_added_cb), NULL);

  g_signal_connect (video_app->decodebin, "pad-added",
      G_CALLBACK (decodebin_pad_added_cb), video_app);

  gst_bin_add_many ((GstBin *) video_app->pipeline, video_app->dynappsrc,
      video_app->decodebin, video_app->playsink, NULL);

  /* Configure appsrc elements in dynappsrc */
  setup_source ();
