/* sample application for testing decproxy with playbin3 in desktop
 *
...
typedef struct
{
  ...
  GstCaps *unsupported_audio_caps;
} GstPlay;
...
static void
element_added_cb (GstElement * pipeline, GstElement * element, gpointer data)
{
    ...
    const gchar * flags_string)
{
  ...
  // use native text sink
  {
    gint f;
    g_object_get (G_OBJECT (play->playbin), "flags", &f, NULL);
    f |= GST_PLAY_FLAG_NATIVE_TEXT;
    f &= ~(GST_PLAY_FLAG_TEXT);
    g_object_set (G_OBJECT (play->playbin), "flags", f, NULL);
  }
  ...
  g_signal_connect (play->playbin, "element-added",
...
play_free (GstPlay * play)
{
  ...
  if (play->unsupported_audio_caps) {
    gst_caps_unref (play->unsupported_audio_caps);
    play->unsupported_audio_caps = NULL;
  }
  g_free (play);
}
  ...
  switch (GST_MESSAGE_TYPE (msg)) {
    ...
    case GST_MESSAGE_STATE_CHANGED:
    {
      if (GST_MESSAGE_SRC (msg) == GST_OBJECT (play->playbin)) {
        ...
        if (oldstate == GST_STATE_NULL && newstate == GST_STATE_READY)
          play->resource_acquisition = FALSE;
    ...
    case GST_MESSAGE_STREAM_COLLECTION:
    {
      ...
      if (collection) {
        ...
        if (play->collection && play->notify_id) {
          g_signal_handler_disconnect (play->collection, play->notify_id);
          play->notify_id = 0;
        }
        ...
        if (play->collection) {
          play->notify_id =
              g_signal_connect (play->collection, "stream-notify",
              (GCallback) stream_notify_cb, play);
        }
      }
      break;
    }
    ...
    case GST_MESSAGE_APPLICATION:
    {
      ...
      if (play->unsupported_audio_caps)
        make_unsupported_codec (play->unsupported_audio_caps);

      if (!play->resource_acquisition) {
        GstStructure *s;
        GstEvent *event;

        play->resource_acquisition = TRUE;
        ...
        gst_element_send_event (play->playbin, event);
      }
      break;
    }
    default:
...
play_do_seek (GstPlay * play, gint64 pos, gdouble rate, GstPlayTrickMode mode)
{
  ...
  if (rate < 0.0 || rate > 1.0)
    seek_flags |=
        GST_SEEK_FLAG_KEY_UNIT | GST_SEEK_FLAG_TRICKMODE |
...
keyboard_cb (const gchar * key_input, gpointer user_data)
{
  ...
  switch (key) {
    ...
    case '<':
      ...
      break;
    case '+':
      if (ABS (play->rate) >= 16.0 || play->rate < 0.0)
        play_set_playback_rate (play, 1.0);
      else
        play_set_playback_rate (play, ABS (play->rate) * 2.0);
      break;
    case '-':
      if (ABS (play->rate) >= 4.0)
        play_set_playback_rate (play, 1.0);
      else
        play_set_playback_rate (play, ABS (play->rate) * -2.0);
      break;
...
main (int argc, char **argv)
{
  ...
  gchar **uris;
  gchar *flags = NULL;
  gchar *unsupported_audio_codec = NULL;
  guint num, i;
  GError *err = NULL;
  GOptionContext *ctx;
  ...
  GOptionEntry options[] = {
    ...
    {"unsupported-audio-codec", 0, 0, G_OPTION_ARG_STRING,
          &unsupported_audio_codec,
        N_("Not supported audio codec"), NULL},
  ...
  if (unsupported_audio_codec) {
    play->unsupported_audio_caps =
        gst_caps_from_string (unsupported_audio_codec);
  }
