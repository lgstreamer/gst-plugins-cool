...
static GType _gst_dvrmpegts_section_type = 0;
#define MPEG_TYPE_TS_SECTION (_gst_dvrmpegts_section_type)
...
static GstDvrMpegtsPatProgram *
_dvrmpegts_pat_program_copy (GstDvrMpegtsPatProgram * orig)
{
  return g_slice_dup (GstDvrMpegtsPatProgram, orig);
}

static void
_dvrmpegts_pat_program_free (GstDvrMpegtsPatProgram * orig)
{
  g_slice_free (GstDvrMpegtsPatProgram, orig);
}

G_DEFINE_BOXED_TYPE (GstDvrMpegtsPatProgram, gst_dvrmpegts_pat_program,
    (GBoxedCopyFunc) _dvrmpegts_pat_program_copy,
    (GFreeFunc) _dvrmpegts_pat_program_free);
...
_parse_pat (GstMpegtsSection * section)
{
  ...
  GstDvrMpegtsPatProgram *program;
  guint8 *data, *end;
  ...
  pat =
      ...
      (GDestroyNotify) _dvrmpegts_pat_program_free);

  GST_LOG ("nb_programs %u", nb_programs);

  for (i = 0; i < nb_programs; i++) {
    program = g_slice_new0 (GstDvrMpegtsPatProgram);
 ...
 *
 * Returns the array of #GstDvrMpegtsPatProgram contained in the section.
 *
 * Note: The PAT "transport_id" field corresponds to the "subtable_extension"
 * field of the provided @section.
 *
 * Returns: (transfer container) (element-type GstDvrMpegtsPatProgram): The
 * #GstDvrMpegtsPatProgram contained in the section, or %NULL if an error
 ...
 * gst_mpegts_pat_new:
 *
 * Allocates a new #GPtrArray for #GstDvrMpegtsPatProgram
 *
 * Returns: (transfer full) (element-type GstDvrMpegtsPatProgram): A newly allocated #GPtrArray
 */
GPtrArray *
 ...
 *
 * Allocates a new #GstDvrMpegtsPatProgram.
 *
 * Returns: (transfer full): A newly allocated #GstDvrMpegtsPatProgram
 */
GstDvrMpegtsPatProgram *
 ...
 * @programs: (transfer full) (element-type GstDvrMpegtsPatProgram): an array of #GstDvrMpegtsPatProgram
...
static GstDvrMpegtsPMTStream *
_gst_dvrmpegts_pmt_stream_copy (GstDvrMpegtsPMTStream * pmt)
{
  GstDvrMpegtsPMTStream *copy;

  copy = g_slice_dup (GstDvrMpegtsPMTStream, pmt);
...
static void
_gst_dvrmpegts_pmt_stream_free (GstDvrMpegtsPMTStream * pmt)
{
  if (pmt->descriptors)
    g_ptr_array_unref (pmt->descriptors);
  g_slice_free (GstDvrMpegtsPMTStream, pmt);
}

G_DEFINE_BOXED_TYPE (GstDvrMpegtsPMTStream, gst_dvrmpegts_pmt_stream,
    (GBoxedCopyFunc) _gst_dvrmpegts_pmt_stream_copy,
    (GFreeFunc) _gst_dvrmpegts_pmt_stream_free);

static GstDvrMpegtsPMT *
_gst_dvrmpegts_pmt_copy (GstDvrMpegtsPMT * pmt)
{
  GstDvrMpegtsPMT *copy;

  copy = g_slice_dup (GstDvrMpegtsPMT, pmt);
  if (pmt->descriptors)
    copy->descriptors = g_ptr_array_ref (pmt->descriptors);
...
static void
_gst_dvrmpegts_pmt_free (GstDvrMpegtsPMT * pmt)
{
  if (pmt->descriptors)
    g_ptr_array_unref (pmt->descriptors);
  if (pmt->streams)
    g_ptr_array_unref (pmt->streams);
  g_slice_free (GstDvrMpegtsPMT, pmt);
}

G_DEFINE_BOXED_TYPE (GstDvrMpegtsPMT, gst_dvrmpegts_pmt,
    (GBoxedCopyFunc) _gst_dvrmpegts_pmt_copy, (GFreeFunc) _gst_dvrmpegts_pmt_free);
...
_parse_pmt (GstMpegtsSection * section)
{
  ...
  pmt->streams =
      ...
      (GDestroyNotify) _gst_dvrmpegts_pmt_stream_free);

  /* parse entries, cycle until there's space for another entry (at least 5
   * bytes) plus the CRC */
  while (data <= end - 4 - 5) {
    GstDvrMpegtsPMTStream *stream = g_slice_new0 (GstDvrMpegtsPMTStream);
 ...
 *
 * Returns the #GstDvrMpegtsPMT contained in the @section.
 *
 * Returns: The #GstDvrMpegtsPMT contained in the section, or %NULL if an error
 * happened.
 */
const GstDvrMpegtsPMT *
gst_mpegts_section_get_pmt (GstMpegtsSection * section)
{
  ...
  if (!section->cached_parsed)
    section->cached_parsed =
        ...
        (GDestroyNotify) _gst_dvrmpegts_pmt_free);

  return (const GstDvrMpegtsPMT *) section->cached_parsed;
}

/**
 * gst_mpegts_pmt_new:
 *
 * Allocates and initializes a new #GstDvrMpegtsPMT.
 *
 * Returns: (transfer full): #GstDvrMpegtsPMT
 */
GstDvrMpegtsPMT *
gst_mpegts_pmt_new (void)
{
  ...
  pmt->streams = g_ptr_array_new_with_free_func ((GDestroyNotify)
      _gst_dvrmpegts_pmt_stream_free);
 ...
 *
 * Allocates and initializes a new #GstDvrMpegtsPMTStream.
 *
 * Returns: (transfer full): #GstDvrMpegtsPMTStream
 */
GstDvrMpegtsPMTStream *
gst_mpegts_pmt_stream_new (void)
{
  GstDvrMpegtsPMTStream *stream;

  stream = g_slice_new0 (GstDvrMpegtsPMTStream);
 ...
 * @pmt: (transfer full): a #GstDvrMpegtsPMT to create a #GstMpegtsSection from
 * @pid: The PID that the #GstDvrMpegtsPMT belongs to
 *
 ...
 */
GstMpegtsSection *
gst_mpegts_section_from_pmt (GstDvrMpegtsPMT * pmt, guint16 pid)
{
  GstMpegtsSection *section;
  ...
  section->destroy_parsed = (GDestroyNotify) _gst_dvrmpegts_pmt_free;
 ...
 *
 * Returns the array of #GstDvrMpegtsDescriptor contained in the Conditional
 * Access Table.
 *
 * Returns: (transfer container) (element-type GstDvrMpegtsDescriptor): The
 * #GstDvrMpegtsDescriptor contained in the section, or %NULL if an error
 ...
 *
 * Returns the array of #GstDvrMpegtsDescriptor contained in the section
 *
 * Returns: (transfer container) (element-type GstDvrMpegtsDescriptor): The
 * #GstDvrMpegtsDescriptor contained in the section, or %NULL if an error
 * happened. Release with #g_array_unref when done.
...
{
  if (_gst_dvrmpegts_section_type)
    return;

  GST_DEBUG_CATEGORY_INIT (mpegts_debug, "dvrmpegts", 0, "DVR MPEG-TS helper library");

  /* FIXME : Temporary hack to initialize section gtype */
  _gst_dvrmpegts_section_type = gst_mpegts_section_get_type ();
