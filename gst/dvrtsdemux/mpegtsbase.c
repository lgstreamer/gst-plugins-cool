...
{
  GstDvrMpegtsPMTStream *pmt = stream->stream;
...
{
  const GstDvrMpegtsPMT *pmt = program->pmt;
...
{
  const GstDvrMpegtsDescriptor *desc;
...
static inline GstDvrMpegtsPMTStream *
_stream_in_pmt (const GstDvrMpegtsPMT * pmt, MpegTSBaseStream * stream)
{
  guint i, nbstreams = pmt->streams->len;

  for (i = 0; i < nbstreams; i++) {
    GstDvrMpegtsPMTStream *pmt_stream = g_ptr_array_index (pmt->streams, i);
...
mpegts_base_update_program (MpegTSBase * base, MpegTSBaseProgram * program,
    GstMpegtsSection * section, const GstDvrMpegtsPMT * pmt)
{
  MpegTSBaseClass *klass = GST_MPEGTS_BASE_GET_CLASS (base);
  gchar *stream_id;
  ...
  for (tmp = program->stream_list; tmp; tmp = next) {
    ...
    GstDvrMpegtsPMTStream *pmt_stream = _stream_in_pmt (pmt, stream);
...
static gboolean
_stream_is_private_section (GstDvrMpegtsPMTStream * stream)
{
...
mpegts_base_is_same_program (MpegTSBase * base, MpegTSBaseProgram * oldprogram,
    guint16 new_pmt_pid, const GstDvrMpegtsPMT * new_pmt)
{
  guint i, nbstreams;
  ...
  for (i = 0; i < nbstreams; ++i) {
    GstDvrMpegtsPMTStream *stream = g_ptr_array_index (new_pmt->streams, i);
...
mpegts_base_deactivate_program (MpegTSBase * base, MpegTSBaseProgram * program)
{
  ...
  if (program->pmt) {
    for (i = 0; i < program->pmt->streams->len; ++i) {
      GstDvrMpegtsPMTStream *stream = g_ptr_array_index (program->pmt->streams, i);
...
mpegts_base_activate_program (MpegTSBase * base, MpegTSBaseProgram * program,
    guint16 pmt_pid, GstMpegtsSection * section, const GstDvrMpegtsPMT * pmt,
...
mpegts_base_apply_pat (MpegTSBase * base, GstMpegtsSection * section)
{
  ...
  if (old_pat) {
    ...
    for (i = 0; i < old_pat->len; ++i) {
      GstDvrMpegtsPatProgram *patp = g_ptr_array_index (old_pat, i);
...
{
  GST_DEBUG_CATEGORY_INIT (mpegts_base_debug, "dvrmpegtsbase", 0,
      "DVR MPEG transport stream base class");
