...
gst_streamid_demux_init (GstStreamidDemux * demux)
{
  ...
  demux->active_collection = NULL;
  g_mutex_init (&demux->collection_lock);

  /* initialize hash table for srcpad */
  g_mutex_init (&demux->output_lock);
...
gst_streamid_demux_dispose (GObject * object)
{
  ...
  gst_object_replace ((GstObject **) & demux->active_collection, NULL);
  g_mutex_clear (&demux->collection_lock);
  g_mutex_clear (&demux->output_lock);
...
static void
update_stream_collection (GstStreamidDemux * demux,
    GstStreamCollection * collection)
{
  guint i;

  GST_COLLECTION_LOCK (demux);

  if (demux->active_collection == collection) {
    GST_LOG_OBJECT (demux, "Already updated collection: %" GST_PTR_FORMAT,
        collection);
    GST_COLLECTION_UNLOCK (demux);
    return;
  }
  ...
  if (demux->active_collection && demux->active_collection != collection) {
    ...
    for (i = 0; i < gst_stream_collection_get_size (demux->active_collection);
        i++) {
      GstStream *stream =
          gst_stream_collection_get_stream (demux->active_collection, i);
      const gchar *osid = gst_stream_get_stream_id (stream);
      GstPad *deactive_srcpad =
          gst_streamid_demux_get_srcpad_by_stream_id (demux, osid);

      if (deactive_srcpad) {
        GST_DEBUG_OBJECT (deactive_srcpad, "trying to remove!!");
        GST_OUTPUT_LOCK (demux);
        g_hash_table_remove (demux->stream_id_pairs, osid);
        gst_pad_set_active (deactive_srcpad, FALSE);
        gst_element_remove_pad (GST_ELEMENT_CAST (demux), deactive_srcpad);
        GST_OUTPUT_UNLOCK (demux);
      }
    }
  }

  gst_object_replace ((GstObject **) & demux->active_collection,
  ...
  for (i = 0; i < gst_stream_collection_get_size (collection); i++) {
    ...
    GST_LOG ("   Stream '%s'", gst_stream_get_stream_id (stream));
    GST_LOG ("     type  : %s",
        gst_stream_type_get_name (gst_stream_get_stream_type (stream)));
    GST_LOG ("     flags : 0x%x", gst_stream_get_stream_flags (stream));
    taglist = gst_stream_get_tags (stream);
    GST_LOG ("     tags  : %" GST_PTR_FORMAT, taglist);
...
gst_streamid_demux_event (GstPad * pad, GstObject * parent, GstEvent * event)
{
  ...
  if (GST_EVENT_TYPE (event) == GST_EVENT_STREAM_START) {
    ...
    if (collection)
      update_stream_collection (demux, collection);
  }
