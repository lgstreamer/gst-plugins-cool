...
GST_START_TEST (test_repeat_state_change)
{
  ...
  for (count = 1; count <= NUM_REPEAT; count++) {
    for (nb_appsrc = 0; nb_appsrc < count; nb_appsrc++) {
      ...
      fail_unless (appsrc[nb_appsrc] != NULL,
          "failed to create appsrc element");
    }
    g_object_get (dynappsrc, "n-source", &n_source, NULL);
...
GST_START_TEST (test_smart_properties)
{
  GstElement *dynappsrc;
  ...
  dynappsrc =
      gst_element_make_from_uri (GST_URI_SRC, "dynappsrc://", "source", NULL);
  g_object_set (dynappsrc, "smart-properties", smart_prop, NULL);
  ...
  g_signal_emit_by_name (dynappsrc, "new-appsrc", NULL, &appsrc2);
...
GST_START_TEST (test_smart_properties2)
{
  GstElement *dynappsrc;
  GstElement *appsrc1, *appsrc2;
  const GstStructure *smart_prop;
  GstStructure *result1 = NULL, *result2 = NULL;
...
dynappsrc_suite (void)
{
  ...
  tcase_add_test (tc_chain, test_smart_properties);
  tcase_add_test (tc_chain, test_smart_properties2);
