...
static gboolean is_resource_mode = TRUE;
    ...
    gpointer user_data)
{
  if (!g_strrstr (GST_ELEMENT_NAME (element), "uridecodebin")
      && !g_strrstr (GST_ELEMENT_NAME (element), "urisourcebin"))
    return;
...
gst_cool_playbin_init (GstElement * playbin)
{
  ...
  GST_DEBUG ("playbin3 is initialized !");

  if (g_strrstr (GST_ELEMENT_NAME (playbin), "playbin3")
      && g_object_class_find_property (G_OBJECT_GET_CLASS (playbin),
          "text-stream-combiner")) {
    GST_DEBUG
        ("playbin3 will have a funnel as a combiner for internal text in Target !");
    GstElement *combiner = gst_element_factory_make ("funnel", NULL);
    g_object_set (G_OBJECT (playbin), "text-stream-combiner", combiner, NULL);
  }
  // FIXME: reclaim connected signal handle when destorying playbin
...
gboolean
gst_cool_playbin_is_resource_mode ()
{
  return is_resource_mode;
}

gboolean
gst_cool_playbin_no_resource_mode (GstElement * playbin,
    gboolean use_decoder_proxy, gboolean is_sync)
{
  ...
  /* Adjust rank for fake audio and video decoders */
  feature = gst_registry_find_feature (gst_registry_get (), "decproxy",
      GST_TYPE_ELEMENT_FACTORY);

  rank =
      use_decoder_proxy ? gst_plugin_feature_get_rank (feature) -
      1 : gst_plugin_feature_get_rank (feature) + 1;

  feature = gst_registry_find_feature (gst_registry_get (), "fakevdec",
      GST_TYPE_ELEMENT_FACTORY);
  ...
  /* Setup all sink as fake with passed sink mode */
  sink = gst_element_factory_make ("fakesink", NULL);
  g_object_set (sink, "sync", is_sync, NULL);
  ...
  sink = gst_element_factory_make ("fakesink", NULL);
  g_object_set (sink, "sync", is_sync, NULL);
  g_object_set (G_OBJECT (playbin), "text-sink", sink, NULL);
