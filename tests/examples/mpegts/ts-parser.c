...
#include <gst/gst.h>
#include <gst/dvrmpegts/mpegts.h>
...
static void
dump_cable_delivery_descriptor (GstDvrMpegtsDescriptor * desc, guint spacing)
{
...
static void
dump_terrestrial_delivery (GstDvrMpegtsDescriptor * desc, guint spacing)
{
...
static void
dump_dvb_service_list (GstDvrMpegtsDescriptor * desc, guint spacing)
{
  GPtrArray *res;
...
static void
dump_logical_channel_descriptor (GstDvrMpegtsDescriptor * desc, guint spacing)
{
...
static void
dump_multiligual_network_name (GstDvrMpegtsDescriptor * desc, guint spacing)
{
  GPtrArray *items;
...
static void
dump_multiligual_bouquet_name (GstDvrMpegtsDescriptor * desc, guint spacing)
{
  GPtrArray *items;
...
static void
dump_multiligual_service_name (GstDvrMpegtsDescriptor * desc, guint spacing)
{
  GPtrArray *items;
...
static void
dump_multiligual_component (GstDvrMpegtsDescriptor * desc, guint spacing)
{
  GPtrArray *items;
  guint8 tag;
...
static void
dump_linkage (GstDvrMpegtsDescriptor * desc, guint spacing)
{
...
static void
dump_component (GstDvrMpegtsDescriptor * desc, guint spacing)
{
  ...
  if (gst_mpegts_descriptor_parse_dvb_component (desc, &res)) {
    ...
    gst_dvrmpegts_dvb_component_descriptor_free (res);
  }
}

static void
dump_content (GstDvrMpegtsDescriptor * desc, guint spacing)
{
  GPtrArray *contents;
  guint i;
...
static void
dump_iso_639_language (GstDvrMpegtsDescriptor * desc, guint spacing)
{
  guint i;
  GstDvrMpegtsISO639LanguageDescriptor *res;
  ...
  if (gst_mpegts_descriptor_parse_iso_639_language (desc, &res)) {
    ...
    gst_dvrmpegts_iso_639_language_descriptor_free (res);
  }
}

static void
dump_dvb_extended_event (GstDvrMpegtsDescriptor * desc, guint spacing)
{
...
dump_descriptors (GPtrArray * descriptors, guint spacing)
{
  ...
  for (i = 0; i < descriptors->len; i++) {
    GstDvrMpegtsDescriptor *desc = g_ptr_array_index (descriptors, i);
    ...
    switch (desc->tag) {
      ...
      case GST_MTS_DESC_DVB_DATA_BROADCAST:
      {
        ...
        if (gst_mpegts_descriptor_parse_dvb_data_broadcast (desc, &res)) {
          ...
          g_printf ("%*s   text              : %s\n", spacing, "",
              res->text ? res->text : "NULL");
          gst_dvrmpegts_dvb_data_broadcast_descriptor_free (res);
        }
        break;
      }
...
{
  const GstDvrMpegtsPMT *pmt = gst_mpegts_section_get_pmt (section);
  guint i, len;
  ...
  g_printf ("     %d Streams:\n", len);
  for (i = 0; i < len; i++) {
    GstDvrMpegtsPMTStream *stream = g_ptr_array_index (pmt->streams, i);
...
{
  const GstDvrMpegtsEIT *eit = gst_mpegts_section_get_eit (section);
  guint i, len;
  ...
  for (i = 0; i < len; i++) {
    ...
    GstDvrMpegtsEITEvent *event = g_ptr_array_index (eit->events, i);
...
dump_atsc_mult_string (GPtrArray * mstrings, guint spacing)
{
  ...
  for (i = 0; i < mstrings->len; i++) {
    GstDvrMpegtsAtscMultString *mstring = g_ptr_array_index (mstrings, i);
    gint j, n;
    ...
    for (j = 0; j < n; j++) {
      GstDvrMpegtsAtscStringSegment *segment =
...
{
  const GstDvrMpegtsAtscEIT *eit = gst_mpegts_section_get_atsc_eit (section);
  guint i, len;
  ...
  g_printf ("     %d Event(s):\n", len);
  for (i = 0; i < len; i++) {
    GstDvrMpegtsAtscEITEvent *event = g_ptr_array_index (eit->events, i);
...
{
  const GstDvrMpegtsAtscETT *ett = gst_mpegts_section_get_atsc_ett (section);
  guint len;
...
{
  const GstDvrMpegtsAtscSTT *stt = gst_mpegts_section_get_atsc_stt (section);
  GstDateTime *dt;
  gchar *dt_str = NULL;

  g_assert (stt);

  dt = gst_mpegts_atsc_stt_get_datetime_utc ((GstDvrMpegtsAtscSTT *) stt);
  if (dt)
...
{
  const GstDvrMpegtsNIT *nit = gst_mpegts_section_get_nit (section);
  guint i, len;
  ...
  g_printf ("     %d Streams:\n", len);
  for (i = 0; i < len; i++) {
    GstDvrMpegtsNITStream *stream = g_ptr_array_index (nit->streams, i);
    g_printf
...
{
  const GstDvrMpegtsBAT *bat = gst_mpegts_section_get_bat (section);
  guint i, len;
  ...
  g_printf ("     %d Streams:\n", len);
  for (i = 0; i < len; i++) {
    GstDvrMpegtsBATStream *stream = g_ptr_array_index (bat->streams, i);
    g_printf
        ("       transport_stream_id:0x%04x , original_network_id:0x%02x\n",
        stream->transport_stream_id, stream->original_network_id);
    dump_descriptors (stream->descriptors, 9);
...
{
  const GstDvrMpegtsSDT *sdt = gst_mpegts_section_get_sdt (section);
  guint i, len;
  ...
  g_printf ("     %d Services:\n", len);
  for (i = 0; i < len; i++) {
    GstDvrMpegtsSDTService *service = g_ptr_array_index (sdt->services, i);
    g_print
...
{
  const GstDvrMpegtsTOT *tot = gst_mpegts_section_get_tot (section);
...
{
  const GstDvrMpegtsAtscMGT *mgt = gst_mpegts_section_get_atsc_mgt (section);
  gint i;

  g_printf ("     protocol_version    : %u\n", mgt->protocol_version);
  g_printf ("     tables number       : %d\n", mgt->tables->len);
  for (i = 0; i < mgt->tables->len; i++) {
    GstDvrMpegtsAtscMGTTable *table = g_ptr_array_index (mgt->tables, i);
    g_printf ("     table %d)\n", i);
...
dump_vct (GstMpegtsSection * section)
{
  ...
  for (i = 0; i < vct->sources->len; i++) {
    GstDvrMpegtsAtscVCTSource *source = g_ptr_array_index (vct->sources, i);
