...
block_startup_buffer_probe (GstPad * pad, GstPadProbeInfo * info,
    GstAppSourceGroup * group)
{
  GST_DEBUG_OBJECT (group->appsrc, "block buffer");

  return GST_PAD_PROBE_OK;
}

static void
remove_blocks (GstDynAppSrc * bin)
{
  GList *item;

  GST_DEBUG_OBJECT (bin, "remove all buffer blocks");
  for (item = bin->appsrc_list; item; item = g_list_next (item)) {
    GstAppSourceGroup *appsrc_group = (GstAppSourceGroup *) item->data;
    if (appsrc_group->block_id) {
      gst_pad_remove_probe (appsrc_group->srcpad, appsrc_group->block_id);
      appsrc_group->block_id = 0;
    }
  }
}
...
setup_source (GstDynAppSrc * bin)
{
  ...
  for (item = bin->appsrc_list; item; item = g_list_next (item)) {
    ...
    appsrc_group->block_id =
        gst_pad_add_probe (appsrc_group->srcpad,
        GST_PAD_PROBE_TYPE_BLOCK | GST_PAD_PROBE_TYPE_BUFFER,
        (GstPadProbeCallback) block_startup_buffer_probe, appsrc_group, NULL);
...
static void
child_appsrc_state_failure_cb (GstBin * bin, GstElement * child)
{
  GST_DEBUG_OBJECT (child, "state change fail, remove buffer block");
  remove_blocks (bin);
}
...
gst_dyn_appsrc_change_state (GstElement * element, GstStateChange transition)
{
  ...
  switch (transition) {
    case GST_STATE_CHANGE_READY_TO_PAUSED:
      ...
      gst_bin_set_state_failure_notify (GST_BIN_CAST (bin),
          (GstBinStateFailureNotifyFunc) child_appsrc_state_failure_cb);
      break;
    default:
      break;
  }
