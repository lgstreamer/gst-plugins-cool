...
#define CONNECTION_ID "aA25cfeEDCB95fa4F8afC8d3828211d"
...
combiner_srcpad_event (GstPad * pad, GstPadProbeInfo * info, GstPlay * play)
{
  ...
  if (GST_EVENT_TYPE (event) == GST_EVENT_CAPS) {
    ...
    if (g_object_class_find_property (G_OBJECT_GET_CLASS (play->video_decoder),
            "shvc-layer")) {
      if (!g_strcmp0 (play->shvc_layer, "bl")) {
        g_print ("      Set shvc-layer: bl \n");
      } else {
        g_print ("      Set shvc-layer: %s \n", play->shvc_layer);
        g_object_set (play->video_decoder, "shvc-layer", play->shvc_layer,
            NULL);
      }
    }
    ...
    g_print ("      Removing pad probe \n");
    play->probe_event_id = 0;
    ...
    gboolean dashmode)
{
  ...
  if (dashmode) {
    ...
    concat_pipeline =
        g_strdup_printf
        ("h265combiner name=hc ! dvbin name=videodecoder quality-layer-id=1 ! videoconvert ! queue ! lxvideosink name=videosink sync=False %s ! dashdemux ! qtdemux ! h265parse disable-passthrough=False ! hc.enhancement_sink %s ! dashdemux ! qtdemux ! h265parse disable-passthrough=False ! hc.base_sink",
        el_location, bl_location);
    pipeline = gst_parse_launch (concat_pipeline, NULL);
  } else {
    g_print ("Constructing filemode ... \n");
    concat_pipeline =
        "filesrc name=bl_filesrc ! qtdemux ! h265parse ! h265combiner name=hc ! dvbin name=videodecoder ! videoconvert ! lxvideosink name=videosink sync=false filesrc name=el_filesrc ! qtdemux ! video/x-h265, stream-format=lhe1 ! h265parse ! hc.enhancement_sink";
    pipeline = gst_parse_launch (concat_pipeline, NULL);
  }
  ...
  srcpad = gst_element_get_static_pad (play->combiner, "src");
  play->probe_event_id =
      gst_pad_add_probe (srcpad, GST_PAD_PROBE_TYPE_EVENT_DOWNSTREAM,
      (GstPadProbeCallback) combiner_srcpad_event, play, NULL);
  ...
  g_print ("[[[[[ try viewctrl \n");
  res = system ("echo 0 0 > /proc/lg/fbdev/viewctrl");
  g_print ("]]]]] viewctrl ret: %d \n", res);

  g_print ("\n [[[[[ try register videooutput \n");
  res =
      system
      ("luna-send -n 1 -f luna://com.webos.service.videooutput/register '{\"appId\":\"com.webos.app.photovideo\",\"context\":\"_sdjoGd6fM2doBu\"}'");
  g_print ("]]]]] register ret: %d \n", res);
  ...
  g_print ("\n [[[[[ try connect videooutput \n");
  res =
      system
      ("luna-send -n 1 -f luna://com.webos.service.videooutput/connect '{\"source\":\"VDEC\",\"sink\":\"MAIN\",\"context\":\"_sdjoGd6fM2doBu\",\"sourcePort\":0}'");
  g_print ("]]]]] connect ret: %d \n", res);

  g_print ("\n [[[[[ try setVideoData for videooutput \n");
  res =
      system
      ("luna-send -n 1 -f luna://com.webos.service.videooutput/setVideoData '{\"context\":\"_sdjoGd6fM2doBu\",\"contentType\":\"movie\",\"frameRate\":50,\"scanType\":\"progressive\",\"width\":3840,\"height\":2160,\"videoInfo\":{\"adaptive\":false,\"path\":\"network\",\"pixelAspectRatio\":{\"width\":1,\"height\":1},\"afd\":16,\"rotation\":\"Deg0\",\"hdrType\":\"none\",\"vui\":{\"transferCharacteristics\":2,\"colorPrimaries\":2,\"matrixCoeffs\":2}}}'");
  g_print ("]]]]] setVideoData ret: %d \n", res);

  g_print ("\n [[[[[ try setDisplayWindow for videooutput \n");
  res =
      system
      ("luna-send -n 1 -f luna://com.webos.service.videooutput/display/setDisplayWindow '{\"context\":\"_sdjoGd6fM2doBu\", \"fullScreen\":true, \"displayOutput\":{\"width\":3840,\"height\":2160,\"x\":0,\"y\":0}, \"sink\":\"MAIN\"}'");
  g_print ("]]]]] setDisplayWindow ret: %d \n", res);
...
play_free (GstPlay * play)
{
  ...
  if (play->probe_event_id) {
    srcpad = gst_element_get_static_pad (play->combiner, "src");
    gst_pad_remove_probe (srcpad, play->probe_event_id);
    play->probe_event_id = 0;
    gst_object_unref (srcpad);
  }
  ...
  res =
      system
      ("luna-send -n 1 -f luna://com.webos.service.videooutput/disconnect '{\"source\":\"VDEC\",\"sink\":\"MAIN\",\"context\":\"_sdjoGd6fM2doBu\",\"sourcePort\":0}'");
  g_print ("disconnect ret: %d \n", res);

  res =
      system
      ("luna-send -n 1 -f luna://com.webos.service.videooutput/unregister '{\"context\":\"_sdjoGd6fM2doBu\"}'");
  g_print ("unregister ret: %d \n", res);

  res = system ("echo 0 1 > /proc/lg/fbdev/viewctrl");
  g_print ("viewctrl ret: %d \n", res);
...
main (int argc, char **argv)
{
  ...
  GOptionEntry options[] = {
    ...
    {"shvc-layer", 0, 0, G_OPTION_ARG_STRING, &shvc_layer,
        N_("shvc layer to use (default is bl)"), NULL},
    {NULL}
  };
